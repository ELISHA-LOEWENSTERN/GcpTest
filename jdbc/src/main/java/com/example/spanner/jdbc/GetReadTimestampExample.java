...
class GetReadTimestampExample {

  static void getReadTimestamp() throws SQLException {
    // TODO(developer): Replace these variables before running the sample.
    String projectId = "my-project";
    String instanceId = "my-instance";
    String databaseId = "my-database";
    getReadTimestamp(projectId, instanceId, databaseId);
  }
  ...
  static void getReadTimestamp(String projectId, String instanceId, String databaseId)
    ...
    try (Connection connection = DriverManager.getConnection(connectionUrl);
      ...
      try (ResultSet rs = statement.executeQuery("SELECT * FROM Singers ORDER BY LastName")) {
        while (rs.next()) {
      ...
      try (ResultSet rs = statement.executeQuery("SHOW VARIABLE READ_TIMESTAMP")) {
        if (rs.next()) {
          System.out.printf("Read timestamp: [%s]%n", rs.getTimestamp("READ_TIMESTAMP").toString());
        }
      }
    }
  }
...
>>>>>>> 62437d39358985a1902e85558a42537ddb92a68d
